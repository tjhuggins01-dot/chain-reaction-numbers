...
export function refillBoard(board: BoardState, rules: RuleSet, spawnPolicy: SpawnPolicy, rng: Rng, idPrefix = 'tile'): BoardState {
  ...
  for (let y = 0; y < board.height; y += 1) {
    for (let x = 0; x < board.width; x += 1) {
      if (tiles[y][x].value === 0) {
        const value = spawnPolicy.nextValue(rng, rules);
